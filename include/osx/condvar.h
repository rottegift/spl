...
spl_cv_timedwait_hires(kcondvar_t *cvp, kmutex_t *mp, hrtime_t tim,
    ...
    const char * const file, const int line, const char * const func);
#define cv_timedwait_hires(cvp, mp, tim, res, flag) \
	spl_cv_timedwait_hires((cvp), (mp), (tim), (res), (flag), __FILE__, __LINE__, __func__)
#define cv_wait(cvp, mp) \
	spl_cv_wait((cvp), (mp), PRIBIO, #cvp, __FILE__, __LINE__, __func__)
#define cv_wait_io(cvp, mp)                     \
	spl_cv_wait((cvp), (mp), PRIBIO, #cvp, __FILE__, __LINE__, __func__)
#define cv_timedwait(cvp, mp, tim)      \
	spl_cv_timedwait((cvp), (mp), (tim), PRIBIO, #cvp, __FILE__, __LINE__, __func__)
#define cv_wait_interruptible(cvp, mp)        \
	spl_cv_wait((cvp), (mp), PRIBIO|PCATCH, #cvp, __FILE__, __LINE__, __func__)
#define cv_timedwait_interruptible(cvp, mp, tim)  \
	spl_cv_timedwait((cvp), (mp), (tim), PRIBIO|PCATCH, #cvp, __FILE__, __LINE__, __func__)
